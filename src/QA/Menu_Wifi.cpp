...
//----------------------------------------------------
void create_wifi_window() {
    WifiWindow = lv_obj_create(NULL);
    lv_scr_load(WifiWindow);
...
//----------------------------------------------------
void create_wifi_scan_window() {
    WifiScanWindow = lv_obj_create(NULL);
    lv_scr_load(WifiScanWindow);
    ...
    if (n == 0) {
        ...
        lv_obj_set_style_text_color(no_networks_label, lv_color_hex(0x000000), LV_PART_MAIN);
    } else {
        // Show network count
        ...
        lv_obj_set_style_text_color(networks_found_label, lv_color_hex(0x000000), LV_PART_MAIN);
    ...
    create_back_button(WifiScanWindow);
}
...
void ssid_selected_event_handler(lv_event_t * e) {
    ...
    lv_obj_t * password_window = lv_obj_create(NULL);
    lv_scr_load(password_window);
    ...
    lv_obj_set_size(password_textarea, 150, 40);  // Set the size of the password input box
    ...
    create_back_button(password_window);
}
...
void connect_to_wifi(const String& ssid, const String& password) {
    ...
    lv_obj_t * connect_window = lv_obj_create(NULL);
    lv_scr_load(connect_window);
    ...
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();  // Don't clear settings here to maintain persistence
    delay(100);
    ...
    while (attempts < maxAttempts) {
        ...
        // Give a little more time for the connection to stabilize
        delay(500);
        status = WiFi.status(); // Check status again after delay
        ...
        switch (status) {
            ...
            case WL_NO_SSID_AVAIL:
                // Double check if we really can't find the network
                delay(1000);
            ...
            case WL_CONNECT_FAILED:
                // Double check if connection succeeded after a brief delay
                delay(1000);
                if (WiFi.status() == WL_CONNECTED) {
                    connected = true;
                    goto connection_complete;
            ...
            case WL_IDLE_STATUS:
                ...
                break;
            default:
                // One final check before reporting an unknown error
                delay(1000);
                if (WiFi.status() == WL_CONNECTED) {
                    connected = true;
                    goto connection_complete;
        ...
        // Update status label with attempt count
...
connection_complete:
    if (connected && WiFi.localIP() != IPAddress(0, 0, 0, 0)) {
        ...
        Serial.println("Saving Wi-Fi credentials to SD card...");
        ...
        lv_obj_set_style_text_color(status_label, lv_color_hex(0x008000), LV_PART_MAIN);
    } else {
        Serial.print("\nFailed to connect: ");
    ...
    // Custom back button: return to WiFi menu instead of main menu
    lv_obj_t * back_btn = lv_btn_create(connect_window);
